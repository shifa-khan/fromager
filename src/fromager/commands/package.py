...
def list_versions(
    ...
    versions: list[Version] = sorted(set(c.version for c in candidates))
    logger.info(f"Found {len(versions)} version(s)")

    if not details:
        cooldown = _resolve_list_versions_cooldown(
            wkctx, req, ignore_per_package_overrides
        )
        if cooldown is not None:
            version_rows = _compute_version_details(
                req.name, candidates, cooldown, provider.supports_upload_time
            )
            versions = [
                Version(r["version"])
                for r in version_rows
                if r["cooldown"] != "blocked"
            ]
    ...
    # --details mode: show upload timestamps, age, and cooldown status
...
# -- export helpers for list-versions --details --------------------------------
